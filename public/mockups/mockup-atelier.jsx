...
function Atelier() {
  return (
    <div style={{ background: AT.sand, color: AT.ink, fontFamily: 'Inter, sans-serif', width: '100%', position: 'relative' }}>
      ...
      <section style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1.05fr 1fr', minHeight: 820 }}>
        ...
        <div style={{ position: 'relative' }}>
          <ImageSlot label="morning light through a gauze curtain — vertical" w="100%" h={820} tone="sand" plate="Plate I" />
          {/* date marker */}
          <div style={{ position: 'absolute', top: 140, left: 64, color: AT.inkSoft, mixBlendMode: 'multiply', zIndex: 2 }}>
        ...
        <div style={{
          ...
          <ATDisplay size={92} italic style={{ color: AT.sageDeep, marginBottom: 36 }}>
            without the dread<span className="ink-bleed" style={{ color: AT.terra }}>.</span>
          </ATDisplay>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 100, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, position: 'relative' }}>
              <ImageSlot className="warm-card" label="a clean wooden desk, in soft morning light" w="100%" h={360} tone="sage" radius={4} plate="Plate II" />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <ImageSlot className="warm-card" label="a fountain pen, resting" w="100%" h={200} tone="sage" radius={4} plate="III" />
                <ImageSlot className="warm-card" label="a small plant, leaning slightly" w="100%" h={200} tone="sage" radius={4} plate="IV" style={{ transform: 'translateY(-22px)' }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ position: 'relative', width: 380, height: 380, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{
              ...
              boxShadow: '0 30px 80px -20px rgba(94,111,95,0.55), inset 0 -10px 30px rgba(60,70,60,0.25), inset 0 10px 30px rgba(255,255,255,0.15)',
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 28 }}>
            {COPY.personas.map((p, i) => {
              ...
              return (
                <article key={i} style={{ display: 'flex', flexDirection: 'column', transform: i % 2 === 1 ? 'translateY(20px)' : 'translateY(0)' }}>
                  <ImageSlot className="warm-card" label={labels[i]} w="100%" h={360} tone={i === 1 || i === 3 ? 'sage' : 'sand'} radius={4} plate={`Plate ${['V','VI','VII','VIII'][i]}`} />
                  <div style={{ padding: '24px 4px 0' }}>
                    <span className="ink-bleed" style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 18, color: AT.terra }}>0{i + 1}.</span>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1.15fr', minHeight: 760 }}>
        ...
        <div style={{ background: AT.sageDeep, padding: '120px 80px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 32, color: AT.paper }}>
          {COPY.pricing.map((p, i) => (
            <div key={i} className="warm-card-deep" style={{
              padding: '40px 36px',
              ...
              <div style={{ position: 'absolute', inset: 0, opacity: 0.18 }} className="grain-bg" />
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 18, position: 'relative' }}>
                ...
                <span className="ink-bleed" style={{ fontFamily: '"Instrument Serif", serif', fontSize: 48, color: AT.terraMist }}>{p.price}</span>
              </div>
              <p style={{ margin: 0, fontFamily: 'Inter', fontWeight: 300, fontSize: 14, lineHeight: 1.7, color: 'rgba(244,240,232,0.82)', position: 'relative' }}>{p.blurb}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: AT.paper, padding: '120px 64px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 40 }}>
            {COPY.trust.map((t, i) => (
              <div key={i} className="warm-card" style={{
                padding: '40px 36px',
                background: AT.sand,
                borderRadius: 12,
      ...
      <section style={{ background: AT.terra, color: AT.paper, padding: '160px 64px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 2 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 100, alignItems: 'center' }}>
            ...
            <div className="warm-card-deep" style={{
              background: AT.paper,
              padding: '48px 40px',
              borderRadius: 8,
              color: AT.ink,
              ...
              <div className="grain-bg" style={{ opacity: 0.4 }} />
              {[
